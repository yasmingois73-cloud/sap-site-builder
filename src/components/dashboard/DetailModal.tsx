...
import type {
  OperacaoDiariaCat,
  OperacaoDiariaLeiturista,
  OperacaoDiariaResponsavel,
  ...
  const leituristasOrdenados = useMemo(() => {
    return [...leituristasFiltrados].sort((a, b) => {
      ...
      if (coluna === "leiturista") {
        comparacao = a.leiturista.localeCompare(b.leiturista, "pt-BR", { sensitivity: "base" });
      } else {
              ...
              <table className="w-full border-collapse text-sm">
                    ...
                    >
                      <div className="flex items-center gap-1">
                        ...
                        {ordenacao.coluna === "leiturista" && (
                          <span>{ordenacao.direcao === "desc" ? "↓" : "↑"}</span>
                        )}
                      </div>
                    </th>
                ...
                <tbody>
                  {leituristasOrdenados.map((l) => {
                    const leituristaDetalhes = l as OperacaoDiariaLeiturista & {
                      codigos?: CodigoUsadoPorLeiturista[];
                      tipo?: string;
                    };

                    const codigosLeiturista = leituristaDetalhes.codigos ?? [];
                    const tipoLeiturista =
                      (leituristaDetalhes.tipo as "leitura" | "repescagem") ?? "leitura";

                    return (
                      <tr key={l.leiturista} className="border-b border-border last:border-0">
                        <td className="px-3 py-2 whitespace-nowrap">
                          <LeituristaCodigos
                            nomeLeiturista={l.leiturista}
                            codigos={codigosLeiturista}
                            tipo={tipoLeiturista}
                          />
                        </td>

                        <td className="px-2 py-2 text-right">{nf.format(l.total_leituras)}</td>
                        ...
                        <td className="px-2 py-2 text-right">{nf.format(l.defeituoso)}</td>
                      </tr>
                    );
                  })}
            ...
            {cats.map((cat) => (
              ...
              >
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-bold">{cat.cat}</p>

                    <p className="text-xs text-muted-foreground">{cat.supervisor}</p>
                  </div>

                  <span className="text-lg font-bold">{cat.percentual_concluido}%</span>
                </div>
                ...
                <div className="mt-4 grid grid-cols-3 gap-3 text-center">
                  <div>
                    ...
                    <p className="mt-1 text-sm font-semibold">{nf.format(cat.total_leituras)}</p>
                  </div>
                ...
                <div className="mt-4 h-2 overflow-hidden rounded-full bg-muted/50">
                  <div
                    ...
                    style={{
                      width: `${Math.min(Math.max(cat.percentual_concluido, 0), 100)}%`,
                    }}
                  />
                </div>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="text-center">
              ...
              <div className="flex justify-center gap-12 text-base font-medium">
                ...
                <span className="text-brand-green">{nf.format(painel.feito)}</span>

                <span className="text-brand-red">{nf.format(painel.falta)}</span>
              </div>
              ...
              <table className="w-full border-collapse text-sm">
                <thead className="sticky top-0 bg-brand-navy text-primary-foreground">
                  <tr className="text-left text-[11px] tracking-wide uppercase">
                    ...
                    <th className="px-2 py-3 text-right font-semibold">Defeituoso</th>
                  </tr>
                </thead>

                <tbody>
                  {responsaveis.map((r) => (
                    <tr key={r.responsavel} className="border-b border-border last:border-0">
                      <td className="px-3 py-2 whitespace-nowrap">
                      ...
                      <td className="px-2 py-2 text-right">{nf.format(r.total_leituras)}</td>

                      <td className="px-2 py-2 text-right text-brand-red">{nf.format(r.falta)}</td>

                      <td className="px-2 py-2 text-right text-brand-green">
                        {nf.format(r.feito)}
                      </td>

                      <td className="px-2 py-2 text-right">{nf.format(r.defeituoso)}</td>
                    </tr>
                  ))}
